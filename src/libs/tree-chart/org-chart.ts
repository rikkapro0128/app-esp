...
class OrgChart {
  ...
  setDragListener() {
    ...
    this.canvasNode.node().ontouchstart = function (e) {
      var touch = e.changedTouches[0];
      self.dragStartPoint_.x = touch.pageX
      self.dragStartPoint_.y = touch.pageY
      self.onDrag_ = true
    }

    this.canvasNode.node().ontouchmove = function (e) {
      var touch = e.changedTouches[0];
      ...
      self.context.translate(
        (touch.pageX - self.dragStartPoint_.x) / self.scale,
        (touch.pageY - self.dragStartPoint_.y) / self.scale
      )
    ...
    this.canvasNode.node().ontouchend = function (e) {
      if (self.onDrag_) {
        self.onDrag_ = false
      }
    }

    this.canvasNode.node().ontouchcancel = function (e) {
      if (self.onDrag_) {
        self.onDrag_ = false
      }
    }
