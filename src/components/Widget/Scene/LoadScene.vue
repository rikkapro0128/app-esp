  ...
  <div>
    <n-collapse>
      <n-collapse-item
        ...
        :title="`Công tắc ${num} - khoá điều khiển ${
          scenes[num]?.value ? scenes[num]?.value.length : 0
        }`"
        :name="num"
      >
        <n-space
          v-if="scenes[num]?.value && scenes[num]?.value.length > 0"
          vertical
        >
          <n-card
            v-for="scene in scenes[num].value"
            :title="`Địa chỉ: ${scene.target}`"
            size="small"
          >
            ...
            <div>
              <p>
                ...
                {{ scene.payload.mode === 1 ? "tất cả" : "chỉ một" }}
              </p>
              <p v-if="scene.payload.mode === 1">
                <b>Vị trí:</b> chạm {{ scene.payload.position }}
              </p>
            </div>
            <span
              ><b>Trạng thái:</b>
              {{ scene.payload.state ? "bật" : "tắt" }}</span
            >
          </n-card>
        </n-space>
...
const onMessage = (event: MessageEvent<any>) => {
  ...
  if (payload) {
    ...
    } else if (message === "SCENE_CREATE_FAILURE") {
      notyf.error(`Thêm ngữ cảnh vào chạm ${position} thất bại!`);
    } else if (message === "SCENE_DELETE_OK") {
      notyf.success(`Xoá ngữ cảnh thành công!`);
      if (position > 0) {
        scenes[position]["value"] = scenes[position]["value"].filter(
          (scene: any) => scene.id !== id
        );
      }
    } else if (message === "SCENE_READ_OK" || message === "SCENE_CREATE_OK") {
