  ...
  >
    <n-drawer-content title="Tạo Automation" closable>
      <n-space vertical>
        ...
        <n-space vertical>
          <n-text strong> Chọn công tắc </n-text>
          <n-tree-select
            v-model:value="selectPosition"
            :options="touchOptions"
            check-strategy="child"
          />
        </n-space>
        <n-space vertical>
          <n-text strong> Chọn trạng thái </n-text>
...
const filterOptionDType = (num: number) => {
  bindOptions.value = Array(num)
    .fill(0)
    .map((_, index) => ({ label: `Chạm ${index + 1}`, value: index + 1 }));
};
...
watch(chooseDType, (dType) => {
  if (dType) {
    touchOptions.value = nodes.value
      .filter(
        (node) =>
          node.info.dType === dType &&
          node.target !== props.nodeBase.id &&
          !node.info.isRoot
      )
      .map((node) => ({
        label: `${node.target}`,
        key: `${node.target}`,
        children: node.info.dType.includes("touch")
          ? node.value.map((touch) => ({
